...
const variantSpecs: Record<TunnelVariantId, TunnelVariantSpec> = {
  ...
  "regent-shell": {
    scene: {
      chamberWidth: { compact: 8, wide: 11 },
      height: { compact: 9, wide: 12 },
      depth: { compact: 24, wide: 32 },
      gap: { compact: 5, wide: 7 },
      tile: { compact: 26, wide: 34 },
      cameraX: 0,
      cameraY: { compact: -6.2, wide: -8.3 },
      cameraDistance: { compact: 10.6, wide: 13.8 },
      ceilingHue: { near: 246, far: 236 },
      wallHue: { near: 250, far: 238 },
      glowHue: { near: 92, far: 76 },
      ceilingLight: { near: 0.31, far: 0.18 },
      wallLight: { near: 0.27, far: 0.15 },
    },
    presentation: {
      frameTop: "rgba(8, 31, 48, 0.96)",
      frameBottom: "rgba(4, 18, 31, 0.99)",
      halo: "rgba(212, 167, 86, 0.16)",
      haze: "rgba(251, 244, 222, 0.11)",
      beamLeft: "22%",
      beamRight: "22%",
      beamOpacity: "0.18",
      beamColor: "rgba(212, 167, 86, 0.48)",
      slitTop: "rgba(251, 244, 222, 0.88)",
      slitBottom: "rgba(212, 167, 86, 0.42)",
      monolithWidth: "clamp(10rem, 14vw, 14rem)",
    },
  },
};
...
function defaultEdgeStroke(depthIndex: number, depth: number, spec: TunnelVariantSpec): string {
  ...
  const blue = Math.round(
    18 + mixValue(spec.scene.glowHue.near, spec.scene.glowHue.far, depthIndex, depth) / 7,
  );
...
function readLayout(root: HTMLElement): TunnelLayout {
  return root.dataset.demo2Layout === "shell" ? "shell" : "study";
}
...
function renderTunnelScene(root: HTMLElement, sceneRoot: HTMLElement): void {
  ...
  const engine = new window.Heerich({
    ...
    camera: {
      ...
      position: [
        spec.scene.cameraX,
        compact ? spec.scene.cameraY.compact : spec.scene.cameraY.wide,
      ],
      distance: compact
        ? spec.scene.cameraDistance.compact
        : spec.scene.cameraDistance.wide,
    },
    style: {
  ...
  if (layout === "shell") {
    addShellFrame(engine, spec, chamberWidth, height, depth, gap);
  } else {
...
function addShellFrame(
  ...
  const centerWidth = gap * 3;
  const totalWidth = sideWidth * 2 + centerWidth;
  const startX = -Math.floor(totalWidth / 2);
  const leftWallX = startX;
  const rightWallX = startX + totalWidth - 1;

  engine.addGeometry({
    type: "box",
    position: [startX, 0, 0],
    size: [totalWidth, 1, depth],
    meta: { zone: "shell", part: "ceiling" },
    style: {
      default: (_x: number, _y: number, z: number) => ({
        fill: ceilingFill(z, depth, spec),
        stroke: defaultEdgeStroke(z, depth, spec),
      ...
      top: (_x: number, _y: number, z: number) => ({
        fill: ceilingFill(z, depth, spec),
        stroke: glowColor(z, depth, spec, 0.2),
        strokeWidth: 0.78,
        filter: "url(#demo2-glow-soft)",
      }),
    },
  ...
  engine.addGeometry({
    type: "box",
    position: [leftWallX, 1, 0],
    size: [1, height, depth],
    meta: { zone: "shell", part: "wall-left" },
    style: {
      default: (_x: number, _y: number, z: number) => ({
        fill: wallFill(z, depth, spec),
        stroke: defaultEdgeStroke(z, depth, spec),
      ...
      right: (_x: number, y: number, z: number) => ({
        fill: wallFill(z, depth, spec),
        stroke: glowColor(z + y * 0.4, depth, spec, 0.12),
        strokeWidth: 0.72,
        filter: "url(#demo2-glow-soft)",
      }),
    },
  ...
  engine.addGeometry({
    type: "box",
    position: [rightWallX, 1, 0],
    size: [1, height, depth],
    meta: { zone: "shell", part: "wall-right" },
    style: {
      default: (_x: number, _y: number, z: number) => ({
        fill: wallFill(z, depth, spec),
        stroke: defaultEdgeStroke(z, depth, spec),
      ...
      left: (_x: number, y: number, z: number) => ({
        fill: wallFill(z, depth, spec),
        stroke: glowColor(z + y * 0.4, depth, spec, 0.12),
        strokeWidth: 0.72,
        filter: "url(#demo2-glow-soft)",
  ...
  engine.addGeometry({
    type: "box",
    position: [startX, 1, depth - 1],
    size: [totalWidth, height, 1],
    meta: { zone: "shell", part: "back" },
    style: {
      default: {
        fill: "rgba(7, 25, 41, 0.92)",
        stroke: "rgba(251, 244, 222, 0.12)",
        strokeWidth: 0.48,
      },
      front: {
        fill: "rgba(251, 244, 222, 0.03)",
        stroke: "rgba(212, 167, 86, 0.46)",
        strokeWidth: 0.62,
        filter: "url(#demo2-glow-soft)",
      },
    },
