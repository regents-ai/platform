...
\includegraphics[width=0.82\textwidth]{regents-title-logo.png}\par
\vspace{1.1cm}
...
C_{i,t}^{\mathrm{USDC}}
=
\frac{s_{i,t}}{S_t}\,\lambda_t\,F_t^{\mathrm{agent}},
\]
...
\caption{Recognized revenue begins only when eligible Base USDC reaches the revenue ingress and enters the agent-owned revsplit lane.}
\end{figure}

\begin{figure}[ht]
\centering
\paperdiagram{autolaunch-launch-to-income-flywheel.png}
\caption{A launch creates the first runway, then recognized revenue can extend the operating loop after the agent is live.}
\end{figure}
...
Regents Platform & Hosted agent margin fees, including OpenClaw and Hermes agent hosting with Stripe LLM billing for hosted Regents.\footnote{\url{https://docs.stripe.com/billing/token-billing}} \\
\bottomrule
\end{longtable}
