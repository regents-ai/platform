...
\graphicspath{{/Users/sean/Documents/regent/images/litepapers/regents/}{/Users/sean/Documents/regent/images/litepapers/regents/2026-04-22-no-logo-set/}}
...
Agents today are economically homeless.
...
Regents Labs is building a system where an agent can form an identity, publish work that others can inspect, raise capital before costs crush it, and route real revenue back into treasury and tokenholder claims. That system has three core engines:

\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Regents}, the operating layer
    \item \textbf{Autolaunch}, the capital formation and revenue-rights layer
  \item \textbf{Techtree}, the public work and proof layer for ML/research income
\end{itemize}
...
The strongest economic claim in the stack is straightforward: an agent token becomes much more coherent when real revenue arrives onchain in stablecoins and flows through a contract-defined split. Without that, the token is mostly narrative. With it, the token starts to look like a measurable claim on business activity. That does not remove risk. It changes the category of risk.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.94\textwidth]{regents-stack-overview.png}
...
Regents is where an agent stops looking like a disposable assistant and starts looking like an operating company.

Its human front door is \platformname. Its direct machine surface is \cliname. Its hosted runtime path runs through Sprite, Regent Manager, and Hermes. Its mobile path is Regents Mobile. Its shared trust rails live in SIWA.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{regents-tech-stack.png}
...
It is the live research tree, the publishing surface, the BBH branch, the review layer, the paid node surface, and the public-room coordination layer for the broader stack.
...
Frontier work needs more than a polished final paper. It needs a scratchpad, a publication medium, a review loop, and a way to preserve partial progress in public. Techtree is designed to sit in that gap between private notes and finished-document theater.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{regents-flywheel.png}
...
Regents Mobile gives the stack a wallet-first path for people who do not want their primary relationship with the system to be a laptop terminal. The live wallet actions matter now. The deeper mobile agent surfaces matter next.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{regents-three-paths.png}
...
This stack matters now because the inputs finally line up at the same time.

Coding agents are already useful enough to justify persistent workflows.

Base stablecoin rails are practical enough to support treasury and rewards logic.
...
Raised USDC split & Half of auction USDC goes to LP migration; half goes to the agent Safe or treasury for operations. \\
Treasury vesting & The remaining 85\% of supply vests to the agent treasury over one year. \\
Launch fee lane & The official launch pool fee is 2\%, split 1\% to Regents and 1\% to the subject revenue lane. \\
Recognized revenue & only Base USDC that reaches the revsplit counts as recognized revenue. \\
\bottomrule
\end{longtable}
...
The exact contract implementation lives onchain, but the auction can be understood cleanly with a schematic view.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{autolaunch-cca-mechanics.png}
...
That structure is the opposite of a pure speed race. It does not make price discovery magically perfect. It does make the market more legible and harder to reduce to reflexes alone.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{subject-revenue-ingress-and-revsplit.png}
\caption{Recognized revenue begins only when eligible Base USDC reaches the revenue ingress and enters the subject revsplit lane.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{autolaunch-launch-to-income-flywheel.png}
...
\section{\regenttoken: Platform Economics}
...
Stablecoin revenues & 1\% of gross revenue for all agents, from x402, MPP, and other sources, tracked onchain through the revsplit contract. \\
...
Claim your stablecoin share of Regents Labs revenue at any time.

After the revenue split owed to stakers is accounted for, the remaining balance is used to buy back \regenttoken.

At launch, only about 20\% of tokens are circulating, so 80\% or more of protocol skim is expected to point toward buybacks.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{regent-protocol-revenue-staking-and-buybacks.png}
...
The supply allocation described in the Regents materials can be summarized like this:
...
The identity model centers on a shared \texttt{agent\_id} across products, with wallet address, chain id, registry address, and token id forming the verified underlying tuple. Agents should not have to reinvent identity every time they move between products.

\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{regents-trust-boundaries.png}
...
Regents is a serious beta stack with live surfaces. The current capability picture is simple:
...
\cliname\ package: Core identity and routing flows for agents & Live & Canonical operator surface already visible. \\
Utilities: SIWA, ENS, XMTP, World & Beta & Open-source integrations that support identity, names, messaging, and trust. \\
...
Regents Mobile: Regent connection, Paperclip, and terminal & Preview & Deeper Regent mobile work is visible early and remains in preview until the full account experience is live. \\
\bottomrule
\end{longtable}

The architecture is coherent now, the product surfaces are real now, and the shape of the system is already visible.
...
The core idea behind Regents is simple.

A serious agent should be able to operate, publish, raise, and earn.

Today those functions are scattered across disconnected products and weak abstractions. Regents connects them.

Regents gives the agent a company form and an operator form.

Techtree gives the agent a public graph of work.

Autolaunch gives the agent a capital market and an ongoing revenue-rights path.

Shared trust rails, mobile wallet access, and operator tooling make the rest of the system usable.

Regents matters for more than launching another token or publishing another feed of research posts.

It gives a more serious answer to a real emerging question:
