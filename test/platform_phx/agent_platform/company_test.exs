defmodule PlatformPhx.AgentPlatform.CompanyTest do
  ...
  test "one human can own multiple companies" do
    ...
    companies = PlatformPhx.AgentPlatform.Companies.list_owned_companies(human)

    assert Enum.map(companies, & &1.id) |> Enum.sort() == Enum.sort([first.id, second.id])

    assert PlatformPhx.AgentPlatform.Companies.get_owned_company(human, " FIRST REGENT ").id ==
             first.id

    assert PlatformPhx.AgentPlatform.Companies.get_owned_company(human, second.id).slug ==
             "second-regent"
  end
  ...
  test "one company can have multiple agents" do
    ...
    owned_company = PlatformPhx.AgentPlatform.Companies.get_owned_company(human, company.id)
