defmodule WebWeb.Api.AgentFormationControllerTest do
  ...
  setup do
    previous_client = Application.get_env(:web, :opensea_http_client)
    previous_responses = Application.get_env(:web, :opensea_fake_responses)
    previous_stripe_client = Application.get_env(:web, :stripe_llm_client)
    previous_sprite_runner = Application.get_env(:web, :agent_platform_sprite_runner)
    ...
    Application.put_env(:web, :opensea_http_client, Web.OpenSeaFakeClient)
    Application.put_env(:web, :opensea_fake_responses, %{})
    Application.put_env(:web, :stripe_llm_client, Web.StripeLlmFakeClient)

    Application.put_env(
      :web,
      :agent_platform_sprite_runner,
      Web.SpriteRunnerFake
    )
    ...
    on_exit(fn ->
      restore_app_env(:web, :opensea_http_client, previous_client)
      restore_app_env(:web, :opensea_fake_responses, previous_responses)
      restore_app_env(:web, :stripe_llm_client, previous_stripe_client)
      restore_app_env(:web, :agent_platform_sprite_runner, previous_sprite_runner)
  ...
  test "billing checkout, webhook sync, and formation queue a company", %{conn: conn} do
    ...
    [job] = all_enqueued(worker: Web.AgentPlatform.Workers.SyncStripeBillingWorker)
